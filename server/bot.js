...
function getHelpText() {
  return [
    'Справка по боту 9950',
    '',
    'Что делает бот:',
    'Бот помогает регистрироваться в системе, открывать mini app, получать смены, подтверждать участие, сообщать об отсутствии, получать напоминания и быстро связываться по сменам.',
    '',
    'Основные команды:',
    '/start - запускает бота, показывает приветствие и главное меню.',
    '/help - открывает полную справку по возможностям бота.',
    '/thanks - открывает готовое сообщение благодарности для @igoldfarb.',
    '',
    'Как работает обычный пользователь:',
    '1. Нажимает "Регистрация в систему" и по шагам отправляет имя, фамилию, телефон, звание и тип службы.',
    '2. После этого заявка уходит администраторам на проверку.',
    '3. После одобрения пользователь может открыть систему и видеть свои смены.',
    '',
    'Что видно пользователю после входа:',
    'Показывается список всех назначенных ему смен, статус ответа по каждой смене, заметки по смене, статистика по отработанным сменам и часам, а во время активной смены также данные о том, кто будет менять.',
    '',
    'Как отвечать на смену:',
    'Можно выбрать "приду", "не уверен" или "не приду".',
    'Если выбрать "не приду", бот попросит написать причину.',
    'После ответа в mini app хранится статус ответа и при необходимости его можно изменить.',
    '',
    'Быстрые ответы прямо в Telegram:',
    'Когда администратор назначает смену или обновляет её, бот может прислать сообщение с кнопками ответа.',
    'Кнопка "приду" сразу подтверждает участие.',
    'Кнопка "не уверен" ставит статус сомнения.',
    'Кнопка "не приду" запрашивает причину отдельным сообщением.',
    '',
    'Напоминания по сменам:',
    'За 15 минут до начала смены бот напоминает тому, кто должен заступить.',
    'За 15 минут до конца активной смены бот отправляет текущему дежурному информацию о следующем человеке, который его заменяет, чтобы можно было быстро связаться.',
    '',
    'Как работает mini app для администратора:',
    'Администратор видит календарь смен.',
    'По нажатию на день открывается окно дня, где можно создать смену, посмотреть существующие смены, отредактировать их, удалить или назначить людей.',
    'Система подсказывает, кого лучше ставить: те, кто давно не дежурил, поднимаются выше, а те, кто только что был на смене, считаются менее предпочтительными.',
    'Также есть защита от пересекающихся смен.',
    '',
    'Как работает одобрение регистрации:',
    'Когда новый человек отправляет анкету, всем администраторам приходит сообщение с кнопками одобрения или отклонения.',
    'После одобрения пользователь получает сообщение, что может заходить в систему.',
    '',
    'Админ-команды:',
    '/addadmin TELEGRAM_ID - назначить пользователя администратором.',
    '/removeadmin TELEGRAM_ID - снять права администратора.',
    'Эти команды работают только у действующих администраторов.',
    '',
    'Что ещё важно:',
    'Бот обновляет username пользователя из Telegram при сообщениях.',
    'Если пользователь уже отправил заявку и она ждёт проверки, повторная регистрация не запускается.',
    'Если уже ожидается причина отсутствия по смене, следующее сообщение пользователя будет сохранено как причина.',
    '',
    'Если что-то не работает:',
    'Попробуй заново отправить /start, открыть систему через кнопку в боте и убедиться, что ты уже одобрен администратором.'
  ].join('\n');
}
